            ...
            if len(tokens[0]) == 4:
              if horaValida(tokens[0]):
                ...
                while len(tokens) != 0:
                  ...
                  if tokens[0][:1] == '(' and tokens[0][2:] == ')' and len(tokens[0]) == 3 and pri == '': #checando prioridade
                    if prioridadeValida(tokens[0]):
                      pri = tokens[0]
                      pri = pri.upper()
                    tokens.pop(0)
                  elif tokens[0][:1] == '@' and contexto == '': #checando contexto
          
                    if contextoValido(tokens[0]):
                      contexto = tokens[0]
                    tokens.pop(0)
                  elif tokens[0][:1] == '+' and projeto == '': #checando o projeto
